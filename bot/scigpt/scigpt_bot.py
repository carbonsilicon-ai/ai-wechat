...
class SciGPTBot(Bot):
    ...
    def __init__(self):
        super().__init__()
        self.sessions = SciGPTSessionManager(SciGPTSession, model=conf().get("model") or "gpt-3.5-turbo")
        self.args = {}
    ...
    def reply(self, query, context: Context = None) -> Reply:
        if context.type == ContextType.TEXT:
            ...
            if not conf().get("text_to_image"):
                logger.warn("[SciGPT] text_to_image is not enabled, ignore the IMAGE_CREATE request")
    ...
    def _chat(self, query, context, retry_count=0) -> Reply:
        ...
        if retry_count > 2:
            ...
            logger.warn("[SciGPT] failed after maximum number of retry times")
            return Reply(ReplyType.TEXT, "请再问我一次吧")

        try:
            # load config
            if context.get("generate_breaked_by"):
                logger.info(f"[SciGPT] won't set appcode because a plugin ({context['generate_breaked_by']}) affected the context")
                app_code = None
            else:
                plugin_app_code = self._find_group_mapping_code(context)
                app_code = context.kwargs.get("app_code") or plugin_app_code or conf().get("scigpt_app_code")
            scigpt_api_key = conf().get("scigpt_api_key")

            session_id = context["session_id"]
            session_message = self.sessions.session_msg_query(query, session_id)
            logger.debug(f"[SciGPT] session={session_message}, session_id={session_id}")
            ...
            logger.info(f"[SciGPT] query={query}, app_code={app_code}, model={body.get('model')}, file_id={file_id}")
            headers = {"Authorization": "Bearer " + scigpt_api_key}

            # do http request
            base_url = conf().get("scigpt_api_base", "https://inplat.drugflow.com/api")
            res = requests.post(url=base_url + "/v1/chat/completions", json=body, headers=headers,
                                timeout=conf().get("request_timeout", 180), verify=False)
            if res.status_code == 200:
                ...
                logger.info(f"[SciGPT] reply={reply_content}, total_tokens={total_tokens}, res_code={res_code}")
                if res_code == 429:
                    logger.warn(f"[SciGPT] 用户访问超出限流配置，sender_id={body.get('sender_id')}")
                else:
                ...
                logger.error(f"[SciGPT] chat failed, status_code={res.status_code}")

                if res.status_code >= 500:
                    # server error, need retry
                    time.sleep(2)
                    logger.warn(f"[SciGPT] do retry, times={retry_count}")
    ...
    def _process_image_msg(self, app_code: str, session_id: str, query:str, img_cache: dict):
        try:
            ...
            if not app_info:
                logger.debug(f"[SciGPT] not found app, can't process images, app_code={app_code}")
                return None
            ...
            msg.prepare()
            logger.info(f"[SciGPT] query with images, path={path}")
    ...
    def _find_group_mapping_code(self, context):
        try:
            if context.kwargs.get("isgroup"):
                ...
                if config.plugin_config and config.plugin_config.get("scigpt"):
                    scigpt_config = config.plugin_config.get("scigpt")
                    group_mapping = scigpt_config.get("group_app_map")
    ...
    def reply_text(self, session: ChatGPTSession, app_code="", retry_count=0) -> dict:
        ...
        try:
            ...
            headers = {"Authorization": "Bearer " + conf().get("scigpt_api_base")}
            ...
            if res.status_code == 200:
                ...
                reply_content = response["choices"][0]["message"]["content"]
                total_tokens = response["usage"]["total_tokens"]
                logger.info(f"[SciGPT] reply={reply_content}, total_tokens={total_tokens}")
                return {
    ...
    def _fetch_app_info(self, app_code: str):
        headers = {"Authorization": "Bearer " + conf().get("scigpt_api_key")}
        # do http request
        base_url = conf().get("scigpt_api_base", "https://inplat.drugflow.com/api")
        ...
        if res.status_code == 200:
            ...
            logger.warning(f"[SciGPT] find app info exception, res={res}")

    def create_img(self, query, retry_count=0, api_key=None):
        try:
            logger.info("[SciGPTImage] image_query={}".format(query))
            headers = {
                "Content-Type": "application/json",
                "Authorization": f"Bearer {conf().get('scigpt_api_key')}"
            }
            data = {
                "prompt": query,
                "n": 1,
            ...
            url = conf().get("scigpt_api_base", "https://inplat.drugflow.com/api") + "/v1/images/generations"
            res = requests.post(url, headers=headers, json=data, timeout=(5, 90), verify=False)
            t2 = time.time()
    ...
    def _fetch_knowledge_search_suffix(self, response) -> str:
        try:
            if response.get("knowledge_base"):
                ...
                logger.info(f"[SciGPT] knowledge base, search_hit={search_hit}, first_similarity={first_similarity}")
                plugin_config = pconf("scigpt")
    ...
    def _fetch_agent_suffix(self, response):
        try:
            plugin_list = []
            logger.debug(f"[SciGPTAgent] res={response}")
            if response.get("agent") and response.get("agent").get("chain") and response.get("agent").get("need_show_plugin"):
                ...
                for turn in chain:
                    ...
                    if i < len(chain) - 1:
                        suffix += "\n"
                    i += 1
                logger.info(f"[SciGPTAgent] use plugins: {plugin_list}")
                return suffix
        except Exception as e:
            logger.exception(e)
...
class SciGPTSessionManager(SessionManager):
    ...
    def session_reply(self, reply, session_id, total_tokens=None, query=None):
        ...
        try:
            ...
            logger.debug(f"[SciGPT] chat history, before tokens={total_tokens}, now tokens={tokens_cnt}")
        except Exception as e:
            logger.warning("Exception when counting tokens precisely for session: {}".format(str(e)))
        return session


class SciGPTSession(ChatGPTSession):
